...
test_parser.add_argument('--alpha', type=float, default=1, help='Weight for the luminance component of the SSIM loss')
test_parser.add_argument('--beta', type=float, default=1, help='Weight for the contrast component of the SSIM loss')
...
test_parser.add_argument('--loss_function', type=str, default='ssim', help='Loss function to use for testing')
# Add argument for the multi-scale feature usage (default False)
test_parser.add_argument('--use_multi_scale', action='store_true', help='Use multi-scale features in the model')
...
def test_model(modalities, model, test_dataset):
    ...
    for images in test_dataset:
        ...
        save_fused_image(modalities, test_output, images['patient_id'], images['slice_number'])  # Save the fused image
